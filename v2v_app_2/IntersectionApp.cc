...
std::map<std::pair<std::string, std::string>, std::vector<CarMessage*>> messages;
std::map<std::string, std::vector<CarMessage>> receivedMessages;
...
std::vector<IntersectMessage*> IntersectionApp::yieldToRight(
...
std::vector<IntersectMessage*> IntersectionApp::yieldToRightLeftTurns(
        std::vector<IntersectMessage*> vehicles) {
    /*
     * This function takes in a list of vehicle messages and figures out who has the right of way
     * based on the yield to right rule of an intersection. It is meant only for cars who are all
...
std::list<const char*> IntersectionApp::priorityCars(
        std::vector<IntersectMessage*> vehicles) {
    /*
...
int IntersectionApp::getDirection(std::string currRoad, std::string nextRoad) {
    /*
...
void IntersectionApp::populateISM(IntersectMessage *ism, bool passed) {
    /*
...
void IntersectionApp::handlePositionUpdate(cObject* obj) {
    ...
    if (simTime() - lastSent >= 1 && !isRSU) {
        ...
        // Store or update the car's position in the map
        carPositions[mobility->getExternalId()] =
                mobility->getCurrentPosition();

        for (const auto& car : carPositions) {
            if (car.first != mobility->getExternalId()) { // Exclude the current car from the comparison
                double distance = mobility->getCurrentPosition().distance(
                        car.second);

                // If the other car is within the range
                if (distance <= 100.0) {

                    std::string senderId = mobility->getExternalId();
                    std::string receiverId = car.first;
                    std::pair<std::string, std::string> carPair =
                            std::make_pair(senderId, receiverId);
                    // Create a new message
                    CarMessage* cm = new CarMessage();
                    std::string carId = mobility->getExternalId();
                    std::string roadId = traciVehicle->getRoadId();
                    std::string greeting = "Hello World from " + carId
                            + " on road " + roadId;
                    cm->setGreeting(greeting.c_str());
                    cm->setCarId(carId.c_str());
                    cm->setRoadId(roadId.c_str());
                    cm->setDistance(distance);

                    messages[carPair].push_back(cm);

                    EV << cm;
                    EV << "#####HELLO WORLD: Car id: " << carId << " roadID :"
                              << roadId << " Distance: " << distance;
                    EV << "#####HELLO WORLD: " << car.first << ", " << carId;

                    // Send message to the other car
                    if (dataOnSch) {
                        if (cm->isScheduled()) {
                            cancelEvent(cm);
                        }
                        scheduleAt(computeAsynchronousSendingTime(1, type_SCH),
                                cm);
                    } else {
                        simtime_t delay = par("delayTimeCars");
                        sendDelayedDown(cm, delay);
                    }
                }
            }
        }
        ...
        for (const auto& item : messages) {
            std::pair<std::string, std::string> carPair = item.first;
            std::vector<CarMessage*> carMessages = item.second;
            EV << "Messages between " << carPair.first << " and "
                      << carPair.second << ":\n";
            for (const auto& msg : carMessages) {
                EV << msg->getGreeting() << " (distance: " << msg->getDistance()
                          << ")\n";
            }
        }
         ...
         */
        if (roadId == "1o" || roadId == "2o" || roadId == "3o"
                || roadId == "4o") {
         ...
         */
        else if (roadId == "1i" || roadId == "2i" || roadId == "3i"
                || roadId == "4i") {
...
void IntersectionApp::handleLowerMsg(cMessage* msg) {
    ...
    if (CarMessage* cm = dynamic_cast<CarMessage*>(wsm)) {
        // Process the received car message
        std::string senderId = cm->getCarId();
        receivedMessages[senderId].push_back(*cm);
    }
    if (IntersectMessage* ism = dynamic_cast<IntersectMessage*>(wsm)) {
//        onISM(ism);
    } else if (RSUMessage* rsm = dynamic_cast<RSUMessage*>(wsm)) {
...
void IntersectionApp::printMessages() {
    // Print the messages received by each car every second
    for (const auto& car : receivedMessages) {
        EV << "Car " << car.first << " has received the following messages:\n";
        for (const auto& msg : car.second) {
            EV << msg.getGreeting() << " (distance: " << msg.getDistance()
                      << ")\n";
        }
    }
